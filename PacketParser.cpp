...
void parse_ip_packet(const u_char *packet, std::string &message)
{
    ...
    message += format_ipv4_address(tcp_packet->ipHdr.srcIp) + ":" + ntohs(tcp_packet->tcpHdr.srcPort) + " > ";
    message += format_ipv4_address(tcp_packet->ipHdr.dstIp) + ":" + ntohs(tcp_packet->tcpHdr.dstPort) + " ";
...
{
    struct tcpHdr *tcp_hdr = reinterpret_cast<const struct tcpHdr*>(packet);
    message += "Flags [";
    ...
    message += "], " + ntohl(tcp_hdr->seqNum) + " ";

    if (tcp_hdr->flags & kACK)
    {
        message += ntohl(tcp_hdr->ackNum) + " ";
    }
